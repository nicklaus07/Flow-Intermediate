...
pub contract KlausToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @KlausToken.Vault
        ...
        // Destroy the vault
        destroy() {
            KlausToken.totalSupply = KlausToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            KlausToken.totalSupply = KlausToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&KlausToken.Minter>(/public/Minter, target: /storage/MinterStorage)
